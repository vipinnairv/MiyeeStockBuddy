...
// A holding that turned within this many trading days is the only thing most
// people came to find out about. Anything older is history, not news.
const SG_FRESH_BARS = 10;

// Split into "worth looking at now" and "everything else". With 30-odd
// holdings, printing a card each turns the answer into a wall to be read
// rather than an answer to be seen.
function _sgSplitByFreshness(ranked, freshBars) {
  const n = freshBars == null ? SG_FRESH_BARS : freshBars;
  const fresh = [], rest = [];
  (Array.isArray(ranked) ? ranked : []).forEach(r => {
    (r && r.signal && r.signal.barsAgo <= n ? fresh : rest).push(r);
  });
  return { fresh, rest };
}

// The whole scan in one sentence, before any detail. This is the answer to
// the question actually being asked - "is there anything I should look at?" -
// and for most scans on most days the honest answer is no.
function _sgHeadline(freshCount, total) {
  if (!total) return { text: 'Nothing to check yet.', tone: 'var(--T3)' };
  if (!freshCount) {
    return {
      text: `Nothing has changed direction recently. All ${total} of your holdings are carrying on as they were.`,
      tone: 'var(--T2)',
    };
  }
  return {
    text: `${freshCount} of your ${total} holdings changed direction in the last couple of weeks.`,
    tone: 'var(--P)',
  };
}

// "The other 34" only makes sense when something was shown above it. With a
// quiet scan there is no "other", just the whole book.
function _sgRestLabel(freshCount, restCount) {
  const plural = restCount === 1 ? '' : 's';
  return freshCount
    ? `Show the other ${restCount} holding${plural}`
    : `Show all ${restCount} holding${plural}`;
}
...
function _sgCard(r) {
  ...
  return `<div style="border:1px solid var(--bd);border-radius:var(--r3);padding:13px 15px;margin-bottom:9px">
...
// Answer first, detail on request. The explanation of what a crossover even
// is, and the list of holdings that did nothing, are both folded away: they
// are worth having, but not worth making everyone read every time.
function _sgPlainHtml(ranked, missing, short, long) {
  const { fresh, rest } = _sgSplitByFreshness(ranked, SG_FRESH_BARS);
  const head = _sgHeadline(fresh.length, ranked.length);

  return `
    <div style="font-size:15px;font-weight:700;color:${head.tone};line-height:1.5;margin-bottom:4px">${head.text}</div>
    <div style="font-size:11.5px;color:var(--T3);margin-bottom:14px">
      Checked ${ranked.length} holding${ranked.length === 1 ? '' : 's'} against their last ${short} and ${long} days.
      ${missing.length ? `${missing.length} had no usable price history.` : ''}
    </div>

    ${fresh.length ? fresh.map(_sgCard).join('') : ''}

    ${rest.length ? `<details style="margin-top:${fresh.length ? '10px' : '0'}">
      <summary style="cursor:pointer;font-size:12.5px;font-weight:700;color:var(--T2);padding:9px 13px;background:var(--BL);border-radius:var(--r3);list-style:none">
        ${_sgRestLabel(fresh.length, rest.length)} ▾
      </summary>
      <div style="margin-top:10px">${rest.map(_sgCard).join('')}</div>
    </details>` : ''}
    ...
    <details style="margin-top:10px">
      ...
      <div style="font-size:12px;color:var(--T3);line-height:1.65;padding:11px 13px">
        It compares each holding's <b>average price over the last ${short} days</b> against its
        <b>average over the last ${long} days</b>. When the shorter one crosses above the longer one, the
        price has been picking up; when it crosses below, it has been fading.
        <br><br>
        It is an old and very simple idea, and on plenty of stocks it works no better than doing nothing,
        which is why each holding also shows what this pattern has actually been worth on <i>that</i>
        stock. If acting on it would have done <i>worse</i> than simply holding, the signal is not telling
        you anything you can profit from, however confident its label looks.
        <br><br>
