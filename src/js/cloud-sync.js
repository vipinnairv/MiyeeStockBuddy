...
async function pmAuthSignIn() {
  ...
  try {
    // Must precede signIn: persistence chosen after the fact would leave the
    // first session stored the wrong way on a shared computer.
    if (typeof window.CloudAuth.setRemember === 'function') {
      await window.CloudAuth.setRemember(_csRememberMe());
    }
    await window.CloudAuth.signIn(email, pass);
...
function pmOpenAccountSettings() {
  ...
  if (!modal) {
    ...
    // Appended inside #section-portfolio, not document.body: the card's
    // colours (--white, --bd, --T1, ...) are CSS custom properties scoped
    // to #section-portfolio, so a modal parented to <body> would render
    // with every var() falling through to nothing - the same invisible-card
    // bug the sign-in gate had before its CSS was moved to the right place.
    const host = document.getElementById('section-portfolio') || document.body;
...
function renderPmAuthGate() {
  ...
  // Two panels: what this is on the left, the form on the right. On a phone
  // the left panel stacks above the form rather than being dropped - it is
  // what tells a first-time visitor why there is a login here at all, on an
  // app whose other half needs no account.
  const feat = (ico, title, sub) => `<div style="display:flex;gap:11px;align-items:flex-start;margin-bottom:13px">
      <div style="flex-shrink:0;width:30px;height:30px;border-radius:8px;background:rgba(255,255,255,.10);
        border:1px solid rgba(255,255,255,.14);display:flex;align-items:center;justify-content:center;font-size:14px">${ico}</div>
      <div>
        <div style="font-size:12.5px;font-weight:700;color:#fff;line-height:1.35">${title}</div>
        <div style="font-size:11.5px;color:rgba(255,255,255,.55);line-height:1.45">${sub}</div>
      </div>
    </div>`;

  gate.innerHTML = `<div class="pm-auth-split">
    <div class="pm-auth-brand">
      <div style="display:flex;align-items:center;gap:11px;margin-bottom:16px">
        <div style="width:42px;height:42px;border-radius:11px;background:var(--P);display:flex;align-items:center;
          justify-content:center;font-size:21px;flex-shrink:0">📊</div>
        <div>
          <div style="font-size:19px;font-weight:800;color:#fff;letter-spacing:-.3px;line-height:1.2">MiyeeStock <span style="color:var(--P)">Buddy</span></div>
          <div style="font-size:10px;font-weight:700;letter-spacing:1.2px;color:rgba(255,255,255,.45);text-transform:uppercase">Portfolio Manager</div>
        </div>
      </div>
      <div style="font-size:16px;font-weight:700;color:#fff;line-height:1.45;margin-bottom:18px">
        Your holdings, on every device you use.
      </div>
      ${feat('🔄', 'Syncs across your devices', 'Add a holding on your laptop, see it on your phone')}
      ${feat('🔒', 'Private to you', 'Only your account can read your portfolio')}
      ${feat('🔑', 'Your API keys never leave', 'Keys stay in this browser and are never uploaded')}
      ${feat('📈', 'Stock Analyser stays open', 'No account needed for charts and analysis')}
    </div>

    <div class="pm-auth-card">
      <div class="pm-auth-title" style="margin-bottom:2px">Sign in</div>
      <div class="pm-auth-sub">Access is by invitation only.</div>

      <label class="pm-auth-lbl" for="pm-auth-email">Email address</label>
      <input id="pm-auth-email" class="form-input" type="email" placeholder="you@example.com" autocomplete="email"
             onkeydown="if(event.key==='Enter')pmAuthSignIn()" style="width:100%">

      <div style="display:flex;align-items:baseline;justify-content:space-between;margin-top:11px">
        <label class="pm-auth-lbl" for="pm-auth-pass" style="margin:0">Password</label>
        <span onclick="pmAuthReset()" style="font-size:11.5px;color:var(--P);cursor:pointer;font-weight:600">Forgot password?</span>
      </div>
      <div style="position:relative">
        <input id="pm-auth-pass" class="form-input" type="password" placeholder="Your password" autocomplete="current-password"
               onkeydown="if(event.key==='Enter')pmAuthSignIn()" style="width:100%;padding-right:38px">
        <span onclick="pmAuthTogglePass()" id="pm-auth-eye" title="Show password"
              style="position:absolute;right:11px;top:50%;transform:translateY(-50%);cursor:pointer;font-size:14px;opacity:.6">👁</span>
      </div>

      <label style="display:flex;align-items:center;gap:8px;margin-top:12px;cursor:pointer;font-size:12px;color:var(--T2)">
        <input id="pm-auth-remember" type="checkbox" checked style="width:15px;height:15px;accent-color:var(--P);cursor:pointer">
        Keep me signed in
      </label>

      <button class="btn btn-pri" style="width:100%;margin-top:14px" onclick="pmAuthSignIn()">Sign in</button>
      <div id="pm-auth-msg" class="pm-auth-msg"></div>

      <div class="pm-auth-tile">
        <div class="pm-auth-tile-title">No account yet?</div>
        <div class="pm-auth-tile-sub">Email <b>miyee.india@gmail.com</b> with your Full Name, Mobile No and
          Email ID. Once your account is created you'll get an email with your password - you can change
          your email or password anytime from Account Settings after signing in.</div>
        <a class="btn btn-sec" style="width:100%;margin-top:9px;display:block;text-align:center;text-decoration:none;box-sizing:border-box"
      ...
      <div style="font-size:10.5px;color:var(--T3);text-align:center;line-height:1.6;margin-top:14px">
        🔒 Your portfolio is private - only your account can read it<br>
        Secured by <b>Firebase Authentication</b> &amp; <b>Firestore</b>
      </div>
    </div>
  </div>`;
}

// Firebase keeps you signed in across browser restarts by default. Unticking
// the box narrows that to the current tab session, which is what a shared or
// public computer needs - so it must be applied BEFORE the sign-in call, not
// after, or the first session is already persisted the wrong way.
function _csRememberMe() {
  const el = document.getElementById('pm-auth-remember');
  return el ? !!el.checked : true;
}

function pmAuthTogglePass() {
  const inp = document.getElementById('pm-auth-pass');
  const eye = document.getElementById('pm-auth-eye');
  if (!inp) return;
  const show = inp.type === 'password';
  inp.type = show ? 'text' : 'password';
  if (eye) { eye.style.opacity = show ? '1' : '.6'; eye.title = show ? 'Hide password' : 'Show password'; }
}
